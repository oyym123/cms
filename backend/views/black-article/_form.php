...
<script type="text/javascript">
    $("#blackarticle-db_id").change(function () {
        var html = '';
        $.ajax({
            url: 'index.php?r=black-article/get-class',
            type: 'GET',
            dataType: 'json',
            data: {db_name: $("#blackarticle-db_id").find("option:selected").text()},
            success: function (msg) {
                $.each(msg, function (key, val) {
                    html += '<option value="' + key + '">' + val + '</option>';
                });
                $("#blackarticle-db_class_id").html(html);
            }
        })
    });
</script>
